...
export default function PresupuestosPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => presupuestosService.getAll({ estado: filtroEstado || undefined, ...filtrosActivos }),
  })
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => navigate('/presupuestos/nuevo')} className="btn-primary flex items-center gap-2">
        ...
        {[{ value: '', label: 'Todos' }, ...Object.entries(ESTADOS).map(([k, v]) => ({ value: k, label: v.label }))].map((f) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-sm transition-colors ${
              filtroEstado === f.value ? 'bg-blue-600 text-white' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'
            }`}
          >
            {f.label}
          </button>
        ...
        <div className="card py-3">
          <div className="flex gap-3 items-end">
            <div>
              ...
              <input type="date" value={filtros.fechaDesde} onChange={(e) => setFiltros((f) => ({ ...f, fechaDesde: e.target.value }))} className="input" />
            </div>
            <div>
              <label className="block text-xs text-gray-500 mb-1">Fecha hasta</label>
              <input type="date" value={filtros.fechaHasta} onChange={(e) => setFiltros((f) => ({ ...f, fechaHasta: e.target.value }))} className="input" />
            </div>
            <button onClick={aplicarFiltros} className="btn-primary">Aplicar</button>
            <button onClick={() => { setFiltros({ fechaDesde: '', fechaHasta: '' }); setFiltrosActivos({}) }} className="btn-secondary">Limpiar</button>
          </div>
        </div>
      )}
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {data?.presupuestos?.map((p) => {
                  const { importeNeto, totalIva } = calcularResumenImpositivo(p.items, p.descuentoGlobal)
                  return (
                    <tr key={p._id} className="border-b border-gray-50 hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 font-medium text-gray-800">{p.cliente?.fantasia || p.cliente?.razonSocial}</td>
                      ...
                      <td className="px-4 py-3 text-gray-600">{formatMoney(importeNeto)}</td>
                      <td className="px-4 py-3 text-gray-500 text-xs">{formatMoney(totalIva)}</td>
                      <td className="px-4 py-3 font-medium">{formatMoney(p.total)}</td>
                      <td className="px-4 py-3">
                        <span className={`badge ${ESTADOS[p.estado]?.class}`}>{ESTADOS[p.estado]?.label}</span>
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1.5">
                          {p.estado !== 'convertido' && (
                            <>
                              {p.estado === 'no_enviado' && (
                                <button onClick={() => cambiarEstado({ id: p._id, estado: 'enviado' })} className="text-xs px-2 py-1 rounded bg-blue-50 text-blue-700 hover:bg-blue-100 transition-colors">
                                  Enviar
                                </button>
                              )}
                              <button onClick={() => navigate(`/pedidos/nuevo?presupuesto=${p._id}`)} className="text-xs px-2 py-1 rounded bg-green-50 text-green-700 hover:bg-green-100 transition-colors">
                                → Pedido
                              </button>
                              <button onClick={() => navigate(`/presupuestos/${p._id}/editar`)} className="text-gray-400 hover:text-blue-600 transition-colors">
                                <Pencil size={14} />
                              </button>
                              <button onClick={() => { if (confirm(`¿Eliminar presupuesto #${p.numero}?`)) eliminar(p._id) }} className="text-gray-400 hover:text-red-500 transition-colors">
                                <Trash2 size={14} />
                              </button>
                            </>
                          )}
