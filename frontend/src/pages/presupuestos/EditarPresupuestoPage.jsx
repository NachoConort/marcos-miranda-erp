...
export default function EditarPresupuestoPage() {
  ...
  useEffect(() => {
    if (!data?.presupuesto || cargado) return;
    ...
    reset({
      ...
      tipoComprobante: p.tipoComprobante || "factura",
      observaciones: p.observaciones || "",
      validezDias: p.validezDias || 30,
      descuentoGlobal: p.descuentoGlobal || 0,
      comisionRepresentacion: p.comisionRepresentacion || {
        porcentaje: 0,
        tipo: "con_factura",
      },
    ...
    items.reduce(
      (a, i) =>
        ...
        (Number(i.precioUnitario) || 0) *
          (Number(i.cantidad) || 0) *
          (1 - Math.min(Number(i.descuento) || 0, 100) / 100),
      0,
    ) *
      (1 - Math.min(Number(descuentoGlobal) || 0, 100) / 100),
    Number(comRepPct),
    comRepTipo,
  );
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <div>
          <h2 className="text-lg font-semibold text-gray-900">
            Editar presupuesto #{data?.presupuesto?.numero}
          </h2>
          <p className="text-xs text-gray-500">
            {data?.presupuesto?.representacion?.fantasia} —{" "}
            {data?.presupuesto?.cliente?.fantasia ||
        ...
        <div className="card-compact">
          ...
          <div className="grid grid-cols-6 gap-2.5">
            ...
            <div>
              ...
              <select {...register("estado")} className="input-sm">
