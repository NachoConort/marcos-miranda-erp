...
import { Plus, Search, Pencil, ToggleLeft, ToggleRight, Trash2, Package, Upload, Download } from 'lucide-react'
...
export default function ProductosPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => productosService.getAll({ busqueda: busquedaActiva || undefined }),
  })
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      {importResult && (
        <div className={`card border ${importResult.resultados.errores.length > 0 ? 'border-yellow-200 bg-yellow-50' : 'border-green-200 bg-green-50'}`}>
          <p className={`text-sm font-medium ${importResult.resultados.errores.length > 0 ? 'text-yellow-800' : 'text-green-800'}`}>
          ...
          <button onClick={() => setImportResult(null)} className="text-xs text-gray-500 hover:underline mt-2">
            Cerrar
          </button>
        </div>
      )}
                ...
                {data?.productos?.map((p) => (
                  <tr key={p._id} className={`border-b border-gray-50 hover:bg-gray-50 ${!p.habilitado ? 'opacity-50' : ''}`}>
                    ...
                    <td className="px-4 py-3 text-gray-500">
                      {p.stockeable ? (
                        <div>
                          <span className={p.cantidadDisponible > 0 ? 'text-green-600 font-medium' : p.aceptaStockNegativo ? 'text-orange-500' : 'text-red-500'}>
                            {p.cantidadDisponible} {p.unidadMedida}
                          </span>
                          {p.aceptaStockNegativo && (
                            <p className="text-xs text-gray-400">Acepta negativo</p>
                          )}
                        </div>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <button onClick={() => navigate(`/productos/${p._id}/editar`)} className="text-gray-400 hover:text-blue-600 transition-colors">
                          <Pencil size={15} />
                        </button>
                        <button
