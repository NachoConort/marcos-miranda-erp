    ...
    busqueda && !seleccionada
      ? options.filter(
          (o) =>
            ...
            (o.sublabel &&
              o.sublabel.toLowerCase().includes(busqueda.toLowerCase())),
        )
      : options;
    ...
    <div className="relative">
      <div className="relative">
        <input
          ...
          className={`${size === "sm" ? "input-sm" : "input"} w-full pr-7 ${disabled ? "opacity-50 cursor-not-allowed" : ""}`}
          autoComplete="off"
        />
        {value && !disabled && (
          <button
        ...
        <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden">
            ...
            {filtradas.length === 0 ? (
              ...
              filtradas.map((opcion, i) => (
                ...
                >
                  ...
                  {opcion.sublabel && (
                    <span
                      className={`text-xs ${i === indiceActivo ? "text-blue-200" : "text-gray-400"}`}
                    >
                      {opcion.sublabel}
                    </span>
                  )}
          ...
          <div className="px-3 py-1.5 border-t border-gray-100 bg-gray-50">
            <p className="text-xs text-gray-400">
              ↑↓ navegar · Enter/Tab seleccionar · Esc cancelar
            </p>
          </div>
        </div>
      )}
